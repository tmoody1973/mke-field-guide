...
6. **Now-playing metadata IS in scope, sourced from the RM playlist app's Neon database** (Tarik's rulings 2026-07-08; stations: 88Nine, HYFIN, Rhythm Lab, 414 Music). Implementation shape (Task 6): a route-handler `/api/now-playing?station=<key>` opens a SECOND Neon connection (`@neondatabase/serverless`, already a dependency) using env `RM_PLAYLIST_DATABASE_URL` — a **read-only role** connection string to the playlist app's database (that DB lives in a different Neon account than mke-events; the string is the hand-off, gitignored `.env` + Vercel env later). The route selects the latest spin for the station (shape TBD against the real schema — likely `SELECT artist, title, played_at FROM <spins> WHERE <station-key> = $1 ORDER BY played_at DESC LIMIT 1`), Zod-validates, responds with `Cache-Control: s-maxage=15`. MiniPlayer polls every ~30s **while playing only**, renders `artist — "title"`, falls back to "Listen live" on any error/absence/staleness (spin older than ~20 min ⇒ treat as no data). Env absent ⇒ route returns the fallback shape (credential-pending pattern — never crashes). Pending from Tarik BEFORE Task 6: (a) read-only pooled connection string into `.env` as `RM_PLAYLIST_DATABASE_URL` (mind the alias-loss precedent when editing `.env`!), (b) the table/column names + station key values, (c) confirmation all four streams are logged near-real-time, (d) the four stream URLs. Controller introspects the schema once the string lands and records exact SQL in the ledger for Task 6's dispatch.
