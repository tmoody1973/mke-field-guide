...
6. **Now-playing metadata IS in scope** (Tarik's ruling 2026-07-08, reversing the earlier trim; he is supplying the stream URLs AND metadata endpoints for all four stations — 88Nine, HYFIN, Rhythm Lab, 414 Music). Implementation shape (Task 6): a route-handler proxy `/api/now-playing?station=<key>` (server-side fetch to the station metadata endpoint, Zod-validated, `Cache-Control: s-maxage=15` — avoids CORS and hides any upstream key), and the MiniPlayer polls it every ~30s **while playing** (no polling when paused), rendering `artist — "title"` in the nowTrack line with graceful fallback to "Listen live" on any error/absence. The station switcher grows to four compact tabs. Exact endpoint URLs/shapes: pending from Tarik — Task 6's implementer transcribes them from the ledger entry the controller records when they arrive; if they have not arrived by Task 6 dispatch, build the proxy against a clearly-marked placeholder config with the fallback path live, and wire real endpoints as a follow-up step in the same task.
