...
6. **Now-playing metadata IS in scope, sourced from the RM playlist app's CONVEX backend** (recon 2026-07-08 of `/Users/tarikmoody/Documents/Projects/rm-playlist-v2` corrected the earlier Neon assumption — v2 runs on Convex; V1 was Supabase; no Neon anywhere). The playlist backend already exposes exactly what we need, public and pre-filtered: query `api.plays.currentByStation({ stationSlug })` → `PublicPlay | null`, station slugs are a validated union `"hyfin" | "88nine" | "414music" | "rhythmlab"`, and the query already filters station-ID spots/promos/legal IDs (`enrichmentStatus "ignored"`) and deleted rows — it powers the app's own public embed widgets. Deployment URL (public): `https://precise-fish-444.convex.cloud`, env `RM_PLAYLIST_CONVEX_URL`. Implementation (Task 6): `npm install convex` (client only), a route-handler `/api/now-playing?station=<slug>` using `ConvexHttpClient` → `client.query(anyApi.plays.currentByStation, { stationSlug })` (use `anyApi` or a locally-declared typed `FunctionReference` — do NOT import the playlist repo's generated api types across repos), Zod-validate the station param against the four-slug union AND the response shape (external data), respond `Cache-Control: s-maxage=15, stale-while-revalidate=30`. MiniPlayer polls every ~30s **while playing only**, renders `artist — "title"`, falls back to "Listen live" on error/null/staleness (playedAt older than ~20 min). Env absent ⇒ fallback shape (credential-pending pattern — never crashes). Still pending from Tarik before Task 6: the four STREAM URLs only (hint from the playlist repo: `HYFIN_ICY_URL`/`RHYTHMLAB_ICY_URL` exist in its .env.local — Icecast mounts likely shared infra; Task 6's curl-verify step stands).
