...
4. **Survivor keys:** += `mad-planet`, `wiggle-room` (8 total). One test case each, following the Task-4 idiom.
...
### Task 2: Tribe events factory + Wiggle Room instance

**Files:**
- Create: `src/ingestion/adapters/html/sources/tribe-events.ts` (the factory — marcus-center.ts's generic logic moves here)
- Modify: `src/ingestion/adapters/html/sources/marcus-center.ts` (becomes a thin instance file OR is deleted with its instance defined in index.ts — pick whichever keeps `selectorParsers['marcus-center']` and every existing marcus test import working UNCHANGED), `src/ingestion/adapters/html/sources/index.ts` (register `wiggle-room`), `src/db/seed.ts` (one row), `src/dedup/confidence.ts` (add `'wiggle-room'`)
- Test: `tests/ingestion/sources-tribe-events.test.ts` (create — wiggle-room + wrapper cases), `tests/dedup/confidence.test.ts` (extend) + fixture `tests/fixtures/html/wiggle-room.txt` (the WRAPPED capture, trimmed to ~5 events verbatim INSIDE the wrapper)
- UNCHANGED: `tests/ingestion/sources-marcus-center.test.ts` (the refactor proof)

- [ ] **Step 1: Fixture** — from scratchpad `wiggleroom-tribe.txt`: keep the `<html><body>` wrapper; trim the inner `events` array to the first 5 events verbatim (adjust `total`/`total_pages` fields only if the parser validates them — prefer not validating them).
- [ ] **Step 2: Failing tests** — (a) wiggle-room instance parses the WRAPPED fixture into records (exact first-record assertion incl. venue from the feed); (b) raw-JSON input still parses (marcus path — cover in the new file with a minimal inline envelope); (c) non-JSON garbage (no braces) THROWS source-identified; (d) confidence case.
- [ ] **Step 3: RED → extract factory → marcus instance (existing tests UNTOUCHED and green) → wiggle-room instance + seed row + key → GREEN.** `npx vitest run tests/ingestion/ tests/dedup/` ALL green; typecheck.
- [ ] **Step 4: Commit** — message `feat: tribe events parser factory — wiggle-room venue source via firecrawl`.

### Ship checklist (finishing pass — controller only)

1. Full gates (test/typecheck/build/e2e). 2. Merge → main, push. 3. `npm run db:seed` (idempotent; verify 19 sources). 4. **`npm run trigger:deploy` MANDATORY** (task-reachable parser code — the standing lesson; verify 7 tasks). 5. NO vercel deploy needed unless README/site files changed (README row additions → deploy anyway, it's cheap). 6. Live ingest `mad-planet` + `wiggle-room` (wiggle-room consumes ~1 firecrawl credit); record honest counts (~5 Mad Planet, ~7+ Wiggle Room). 7. Spot-check venues by SQL. 8. Evidence + close the slice issue.
