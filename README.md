...
**Provenance.** Every field change made through the editor writes one row to `event_edits` (`event_id` FK cascades on event delete, `edited_by` the verified staff email, `field`, `old_value`, `new_value`, `created_at` — same durability posture as the `event_clusters` merge receipt from Slice 2) — shown as an edit-history list on the event's editor page, newest first. Because the Neon HTTP driver has no transactions, the two mutation paths order their writes deliberately for different failure modes: `updateEventWithDb` and `unlockFieldWithDb` write the audit row *before* the field update, so a crash between them produces a duplicate history row on retry rather than a silently-lost lock; `updateInstanceTimeWithDb` moves the instance, then locks `time`, then writes the audit row — locking before auditing shrinks the window where a crash would leave a genuinely-moved, unlocked time for the next ingest to silently revert, at the cost of possibly losing one (cosmetic) history row instead.

**Riders folded into this slice:** the subscription throttle prunes stale `subscription_attempts` rows in bounded batches of `PRUNE_BATCH = 500` per request rather than an unbounded scan (`src/lib/subscribe-throttle.ts`); the staff allowlist parser (`src/lib/staff-auth.ts`) validates entry shape (`@domain` rules must look like a domain, bare emails must contain `@` with a non-empty local part) and drops anything malformed with a `console.warn` — fail closed, so a typo'd entry matches no one instead of silently matching everyone or crashing; Playwright's admin e2e workers are pinned to 2 locally (`playwright.config.ts`, still 1 in CI) to cut flakiness from shared-dev-server contention; and the `Db` type now has one canonical home (`src/db/types.ts`), with `src/lib/card-data.ts` and `src/ingestion/persist.ts` re-exporting it for their existing importers rather than each declaring their own.
