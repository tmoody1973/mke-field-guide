...
**Rep-vs-Pabst pairs.** Milwaukee Rep publishes all-day midnight instances since no per-performance time is fetchable, and a midnight placeholder disables the same-show window — so a Rep production also billed under Pabst Theater (e.g. *A Christmas Carol*) lands in the review queue around 0.775 with null time evidence instead of auto-merging, and because both sides are venue-owned the survivor ladder falls back to oldest-event rather than favoring either listing. Prefer the timed side (Pabst) as survivor: the merged event will carry both all-day and timed instances, so prune the redundant all-day rows after merging.
