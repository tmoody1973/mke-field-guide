...
**ANNOTATE-ONLY.** The judge never merges, never rejects, and never touches `event_reviews.status` — a human still resolves every pair through the existing approve/reject flow in `/admin/review`. `recordJudgment` writes only the four `judge_*` columns (`judgeVerdict`, `judgeConfidence`, `judgeRationale`, `judgedAt`), and the `UPDATE` is itself guarded on `status = 'pending' AND judged_at IS NULL` so a pair a human already resolved — or that cascaded away via a same-show merge — mid-sweep can never be overwritten; the sweep reports an honest `judged`/`skipped` split rather than assuming every fetched row landed. `judgePair` never throws (any model, network, or validation failure returns `null`, leaving `judgedAt` untouched for retry next sweep), and `dedupSweep` additionally wraps the whole judge call in its own `try/catch` — a gateway outage annotates zero pairs but never fails the cron tick or discards that run's merge/queue counts.
